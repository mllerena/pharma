 ...
 */
@Stateless
public class MaWorkstationFacade extends AbstractFacade<MaWorkstation> {
    ...
    public MaWorkstationFacade() {
        super(MaWorkstation.class);
    }
    
    public List<MaWorkstation> find(MaWorkstation filter) {

		System.out.println("filter: " + filter);

		List<MaWorkstation> result = null;

		StringBuilder sql = new StringBuilder();
		sql.append("SELECT e FROM MaWorkstation e ");
		sql.append("WHERE ");
		sql.append("e.maWorkstationId like :id ");
		sql.append("AND ");
		...
		if ( filter.getMaSectionId() != null ){
			sql.append("AND ");
			sql.append("e.maSectionId like :maSectionId ");
		}
		...
		try {
			...
			if ( filter.getMaSectionId() != null )
			query.setParameter("maSectionId", filter.getMaSectionId() ); 
    ...
    public MaWorkstation findByName(String name) {
        List<MaWorkstation> result = null;
        try {
            Query query = em.createNamedQuery("MaWorkstation.findByName");
    ...
    public void save(MaWorkstation entity) throws ExecuteRollbackException {
        try {
        	...
        	throw new ExecuteRollbackException("El puesto de trabajo "+ entity.getName() +" ya existe.");
        } catch (Exception e) {
        	e.printStackTrace();
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    public void delete(MaWorkstation entity) throws ExecuteRollbackException {
        try {
        	if( entity == null ){
    ...
    // =============================================================
    public void validarExistencia(MaWorkstation entity) throws ExistException {
        try {
        	MaWorkstation temp = findByName(entity.getName());
            if (temp != null) {
    ...
    public void validarExistenciaActualizar(MaWorkstation entity) throws ExistException {
        try {
        	MaWorkstation entityWithNotChange = find(entity.getId());
